...
type PortResult = {
  ...
  web: { reachable: boolean; status: number | null; statusText: string | null; title: string | null; server: string | null; contentType: string | null; url: string; error: string | null };
};
...
function PortTable({ results, selectedPort, onSelect, search, setSearch, filter, setFilter }: { results: PortResult[]; selectedPort: number | null; onSelect: (port: number) => void; search: string; setSearch: (value: string) => void; filter: "all" | "used" | "free"; setFilter: (value: "all" | "used" | "free") => void }) {
  ...
  return <div className="ports-panel">
    ...
    <div className="table-scroll"><table><thead><tr><th>Port</th><th>Status</th><th>Process</th><th>Runtime</th><th>Web</th><th /></tr></thead><tbody>
      {visible.length === 0 ? <tr><td colSpan={6} className="table-empty"><Search size={18} /><span>No matching ports</span></td></tr> : visible.map((result) => <tr key={result.port} className={selectedPort === result.port ? "row-selected" : ""} onClick={() => onSelect(result.port)}><td><span className="port-number">{result.port}</span><span className="protocol">{result.protocol}</span></td><td><StatusPill state={result.state} /></td><td><div className="table-process"><span className="mini-process"><FileCode2 size={14} /></span>{result.process?.command || "—"}</div></td><td><span className="runtime-text">{result.process?.runtime || "—"}</span></td><td>{result.web.reachable ? <span className="web-badge"><Globe2 size={13} />HTTP {result.web.status}</span> : <span className="muted-text">—</span>}</td><td><ChevronRight className="row-arrow" size={15} /></td></tr>)}
...
function DetailSheet({ result, onClose }: { result: PortResult | null; onClose: () => void }) {
  ...
  return <aside className="detail-sheet"><div className="sheet-handle" /><div className="detail-heading"><div><p className="eyebrow">Port detail</p><h2>{result.port}</h2></div><button className="icon-button" aria-label="Close detail" onClick={onClose}><X size={17} /></button></div><div className="detail-status-row"><StatusPill state={result.state} /><span className="muted-label">{result.protocol} · {result.address}</span></div>
    ...
    <div className="detail-section"><div className="section-label"><Globe2 size={15} />Web service</div><div className="web-card"><div className="web-row"><span>URL</span><code>{result.web.url}</code><button className="copy-button" aria-label="Copy URL" onClick={copyUrl}>{copied ? <Check size={13} /> : <Clipboard size={13} />}</button></div><div className="web-row"><span>Status</span><strong className={result.web.reachable ? "success-text" : "muted-text"}>{result.web.status ? `${result.web.status} ${result.web.statusText ?? ""}` : "Not detected"}</strong></div><div className="web-row"><span>Title</span><strong>{result.web.title || "—"}</strong></div><div className="web-row"><span>Content type</span><strong>{result.web.contentType || "—"}</strong></div></div><button className="open-service" disabled={!result.web.reachable} onClick={() => window.open(result.web.url, "_blank", "noopener,noreferrer")}><ExternalLink size={15} />Open web service<ArrowUpRight size={14} /></button></div>
  </aside>;
}
