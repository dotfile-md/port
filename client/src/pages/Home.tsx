...
type PortFilter = "all" | "used" | "free" | "favorite";
type SortMode = "port" | "status" | "runtime";
type PortResult = {
  port: number;
  state: "used" | "free";
  protocol: "TCP";
...
function PortTable({ results, selectedPort, onSelect, search, setSearch, filter, setFilter, favorites, onToggleFavorite, sort, setSort }: { results: PortResult[]; selectedPort: number | null; onSelect: (port: number) => void; search: string; setSearch: (value: string) => void; filter: PortFilter; setFilter: (value: PortFilter) => void; favorites: number[]; onToggleFavorite: (port: number) => void; sort: SortMode; setSort: (value: SortMode) => void }) {
  const visible = results.filter((result) => {
    const term = search.toLowerCase().trim();
    const matchesFilter = filter === "all" || (filter === "favorite" ? favorites.includes(result.port) : result.state === filter);
    return matchesFilter && (!term || String(result.port).includes(term) || result.process?.command?.toLowerCase().includes(term) || result.process?.runtime?.toLowerCase().includes(term));
  }).sort((a, b) => sort === "port" ? a.port - b.port : sort === "status" ? a.state.localeCompare(b.state) : (a.process?.runtime ?? "").localeCompare(b.process?.runtime ?? ""));
  return <div className="ports-panel">
    <div className="panel-header table-heading"><div><h2>Port activity</h2><p>{visible.length} of {results.length} ports shown</p></div><div className="table-actions"><label className="search-box"><Search size={15} /><input aria-label="Search ports" placeholder="Search ports" value={search} onChange={(event) => setSearch(event.target.value)} /></label><select className="sort-select" aria-label="Sort ports" value={sort} onChange={(event) => setSort(event.target.value as SortMode)}><option value="port">Sort: Port</option><option value="status">Sort: Status</option><option value="runtime">Sort: Runtime</option></select><div className="filter-group">{(["all", "used", "free", "favorite"] as const).map((value) => <button key={value} className={filter === value ? "selected" : ""} onClick={() => setFilter(value)}>{value === "favorite" ? "Pinned" : value[0].toUpperCase() + value.slice(1)}</button>)}</div></div></div>
...
function DetailSheet({ result, onClose }: { result: PortResult | null; onClose: () => void }) {
  ...
  const copyCommand = async () => { if (!result.process?.args) return; await navigator.clipboard?.writeText(result.process.args); setCopiedCommand(true); window.setTimeout(() => setCopiedCommand(false), 1400); };
  return <aside className="detail-sheet"><div className="sheet-handle" /><div className="detail-heading"><div><p className="eyebrow">Port detail</p><h2>{result.port}</h2></div><button className="icon-button" aria-label="Close detail" onClick={onClose}><X size={17} /></button></div><div className="detail-status-row"><StatusPill state={result.state} /><span className="muted-label">{result.protocol} · {result.address}</span></div>
    <div className="detail-section"><div className="section-label"><TerminalSquare size={15} />Process</div>{result.process ? <div className="process-card"><div className="process-main"><div className="process-avatar"><FileCode2 size={18} /></div><div className="process-copy"><strong>{result.process.command || "Unknown process"}</strong><span>{result.process.args || "Command line unavailable"}</span></div><button className="copy-button" aria-label="Copy process command" onClick={copyCommand}>{copiedCommand ? <Check size={13} /> : <Clipboard size={13} />}</button></div><div className="process-meta"><span><b>PID</b>{result.process.pid ?? "—"}</span><span><b>Runtime</b>{result.process.runtime ?? "Unknown"}</span><span><b>User</b>{result.process.user ?? "—"}</span><span><b>Uptime</b>{result.process.uptime ?? "—"}</span></div></div> : <div className="soft-empty">No listener process was found.</div>}</div>
...
export default function Home() {
  ...
  const [search, setSearch] = useState(""); const [filter, setFilter] = useState<PortFilter>("all"); const [sort, setSort] = useState<SortMode>("port"); const [selectedPort, setSelectedPort] = useState<number | null>(null); const [lastScan, setLastScan] = useState(now());
  ...
  const [scanHistory, setScanHistory] = useState<string[]>(() => { try { return JSON.parse(localStorage.getItem("portscope:history") ?? "[]"); } catch { return []; } });
  ...
  const runScan = () => { const request: ScanRequest = mode === "popular" ? { mode } : mode === "range" ? { mode, start: Number(start), end: Number(end) } : { mode, ports: customPorts.split(",").map((port) => Number(port.trim())).filter(Boolean) }; const stamp = new Date().toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" }); setScanHistory((current) => [stamp, ...current.filter((item) => item !== stamp)].slice(0, 5)); setSelectedPort(null); setScanRequest(request); setLastScan(stamp); };
  ...
  useEffect(() => { localStorage.setItem("portscope:history", JSON.stringify(scanHistory)); }, [scanHistory]);
